...
function buildQuoteEstimate(data) {
  ...
  const specialRoutePricing = getSpecialRouteTransportPricing(data.origin, data.destination);

  if (specialRoutePricing) {
    return {
      ...
      routeCharge: specialRoutePricing.providerCost,
      weightCharge: 0,
      cargoCharge: specialRoutePricing.providerCost,
      insurance: 0,
      platformFee: specialRoutePricing.profit,
      total: specialRoutePricing.customerTotal,
      serviceWindow: "24-48 h",
      specialRoutePricing
    };
  }
...
function getSpecialRouteTransportPricing(origin, destination) {
  const originText = normalizeText(origin || "");
  const destinationText = normalizeText(destination || "");
  const brazucoToPunta =
    originText.includes("brazuco") && destinationText.includes("punta del este");
  const puntaToBrazuco =
    originText.includes("punta del este") && destinationText.includes("brazuco");

  if (brazucoToPunta) {
    const providerCost = 1900;
    const profit = roundCurrency(providerCost * 0.28);
    return {
      providerCost,
      profit,
      customerTotal: roundCurrency(providerCost + profit),
      routeLabel: "Brazuco -> Punta del Este"
    };
  }

  if (puntaToBrazuco) {
    const providerCost = 1700;
    const profit = roundCurrency(providerCost * 0.28);
    return {
      providerCost,
      profit,
      customerTotal: roundCurrency(providerCost + profit),
      routeLabel: "Punta del Este -> Brazuco"
    };
  }
...
function renderQuoteEstimate(container, data) {
  ...
  const specialRoute = estimate.specialRoutePricing;
    ...
    <article class="quote-result-card">
      ...
      <div class="quote-breakdown">
        ${
          specialRoute
            ? `
              <div class="quote-breakdown-row"><span>Costo transporte</span><strong>${formatCurrency(specialRoute.providerCost)}</strong></div>
              <div class="quote-breakdown-row"><span>Ganancia 28%</span><strong>${formatCurrency(specialRoute.profit)}</strong></div>
              <div class="quote-breakdown-row"><span>Total cotizado</span><strong>${formatCurrency(specialRoute.customerTotal)}</strong></div>
            `
            : `
