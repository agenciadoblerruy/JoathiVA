...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!marketingAllowed) {
    window.location.href = user && typeof getLandingRoute === "function" ? getLandingRoute(user) : "login.html";
    return;
  }
...
function buildMediaChecklist(brief) {
  ...
  return [
    ...
    `0.3 Datos comerciales: ${getMediaContactItems(brief).join(" | ") || "sin web, mail ni WhatsApp cargados"}.`,
...
function renderMediaPlatformSpec(sourceBrief = null) {
  ...
  node.innerHTML = [
    ...
    {
      label: "Logos",
      title: getMktjoathiLogoSummary().length ? "Assets cargados" : "Pendiente de logo manual",
      body: getMktjoathiLogoSummary().join(" | ") || "mktjoathi reservara espacio para aplicar el logo aprobado en edicion final."
    }
  ].map((item) => `
...
function buildMediaContext(brief) {
  ...
  return [
    ...
    `Assets locales: ${getMktjoathiLogoSummary().join(" | ") || "sin logos cargados en mktjoathi"}.`,
    "Los logos cargados son locales del navegador y no se adjuntan al backend; la respuesta debe indicar como aplicarlos manualmente sin recrearlos.",
