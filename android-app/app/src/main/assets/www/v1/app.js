  ...
  switch (action) {
    case "enter-app":
      setSession({
        ...
        role: target.getAttribute("data-role") || "Comercial"
      });
      navigate("home");
      break;
    case "logout":
...
function renderShellFrame(routeName) {
  ...
  appNav.innerHTML = NAV_ITEMS.map(({ route, label }) => {
    ...
    <section class="page">
      <div class="page-hero">
        <div class="hero-copy">
          ...
          <p>
            JoathiVA V1 centraliza CRM, cotizaciones, agenda y ficha de cliente en una experiencia premium,
            limpia y pensada para vender mas con menos friccion.
          </p>
          <div class="hero-actions">
            <button type="button" class="btn btn--accent" data-action="enter-app" data-role="Comercial">Entrar al panel</button>
...
function exportBackupJson() {
  ...
  uiState.backupFeedback = {
    ...
    details: `${fileName} incluye ${backup.customers.length} clientes, ${backup.operations.length} operaciones y ${backup.activityLog.length} actividades.`
  };
  renderApp(false);
}
...
function importBackupForm(form) {
  ...
  uiState.backupFeedback = {
    ...
    details: `Operaciones: ${result.countsBefore.operations} -> ${result.countsAfter.operations}. Clientes: ${result.countsBefore.customers} -> ${result.countsAfter.customers}. Actividad: ${result.countsBefore.activityLog} -> ${result.countsAfter.activityLog}.`
  };
  renderApp(false);
}
