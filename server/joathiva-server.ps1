...
if (-not (Test-Path $DbPath)) {
  throw "No se encontrÃ³ la base JSON en $DbPath"
}
...
. (Join-Path $PSScriptRoot "pg-adapter.ps1")
. (Join-Path $PSScriptRoot "pg-endpoints.ps1")
...
function Resolve-StaticPath {
  ...
  if ((Split-Path -Leaf $ServerRoot) -eq "V" -and $relativePath.StartsWith("V\", [System.StringComparison]::OrdinalIgnoreCase)) {
...
try {
  while ($true) {
    ...
    try {
      ...
      # --- PostgreSQL endpoints inline ---
      if ($path.StartsWith("/api/pg")) {
        Write-Host "DEBUG PG HIT: $path"
        Write-Host "DEBUG PG HIT: $path"
        try {
          $env:PGPASSWORD = "joathi2026"
          $pgPath = $path.TrimEnd("/")
          if ($pgPath -eq "/api/pg/health") {
            $r = & psql -h 127.0.0.1 -U joathi -d joathi_db -t -A -c "SELECT 1" 2>&1
            Write-JsonResponse -Stream $stream -Body @{ ok = ($r -contains "1"); source = "postgresql" }
          } elseif ($pgPath -eq "/api/pg/customers") {
            $r = & psql -h 127.0.0.1 -U joathi -d joathi_db -t -A -c "SELECT json_agg(q) FROM (SELECT id,name,email,phone,city,country,status FROM customers ORDER BY name) q" 2>&1
            $data = if ($r) { $r | ConvertFrom-Json } else { @() }
            Write-JsonResponse -Stream $stream -Body @{ ok = $true; data = $data }
          } elseif ($pgPath -eq "/api/pg/operations") {
            $r = & psql -h 127.0.0.1 -U joathi -d joathi_db -t -A -c "SELECT json_agg(q) FROM (SELECT o.id,o.status,o.reference,o.dua,o.origin,o.destination,o.arrival_date,o.risk,c.name as customer_name FROM operations o LEFT JOIN customers c ON c.id=o.customer_id ORDER BY o.created_at DESC) q" 2>&1
            $data = if ($r) { $r | ConvertFrom-Json } else { @() }
            Write-JsonResponse -Stream $stream -Body @{ ok = $true; data = $data }
          } elseif ($pgPath -eq "/api/pg/providers") {
            $r = & psql -h 127.0.0.1 -U joathi -d joathi_db -t -A -c "SELECT json_agg(q) FROM (SELECT id,name,email,phone,status FROM providers ORDER BY name) q" 2>&1
            $data = if ($r) { $r | ConvertFrom-Json } else { @() }
            Write-JsonResponse -Stream $stream -Body @{ ok = $true; data = $data }
          } elseif ($pgPath -eq "/api/pg/brokers") {
            $r = & psql -h 127.0.0.1 -U joathi -d joathi_db -t -A -c "SELECT json_agg(q) FROM (SELECT id,name,email,phone,company,country FROM brokers ORDER BY name) q" 2>&1
            $data = if ($r) { $r | ConvertFrom-Json } else { @() }
            Write-JsonResponse -Stream $stream -Body @{ ok = $true; data = $data }
          } else {
            Write-JsonResponse -Stream $stream -StatusCode 404 -ReasonPhrase "Not Found" -Body @{ ok = $false; error = "PG endpoint not found" }
          }
          $env:PGPASSWORD = ""
          continue
        } catch {
          $env:PGPASSWORD = ""
          Write-JsonResponse -Stream $stream -StatusCode 500 -ReasonPhrase "Internal Server Error" -Body @{ ok = $false; error = [string]$_.Exception.Message }
          continue
        }
      }
      # --- fin PostgreSQL endpoints ---
